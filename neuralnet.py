...
class Sigmoid:
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO: implement
        return dz*self.sigmoid_value*(1-self.sigmoid_value)
        raise NotImplementedError
...
class Linear:
    def __init__(self, input_size: int, output_size: int,
        ...
        # TODO: Initialize any additional values you may need to store for the
        #  backward pass here
        self.linear_value=np.zeros(output_size)
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        # TODO: perform forward pass and save any values you may need for
        #  the backward pass
        self.value_with_bias = np.append(1, x)
        self.linear_value=np.dot(self.weights, self.value_with_bias)
        return self.linear_value
        raise NotImplementedError
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        # TODO: implement
        dx=np.dot(self.weights[:,1:].T, dz)
        self.dw=np.outer(dz,self.value_with_bias)
        return dx
        raise NotImplementedError
...
class NN:
    ...
    def forward(self, x: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        #  Hint: you may want to use the functions you implemented above
        a=self.linear1.forward(x)
        b=self.sigmoid.forward(a)
        c=self.linear2.forward(b)
        y_hat,loss=self.softmax.forward(c, y)
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> None:
        ...
        a=self.softmax.backward(y, y_hat)
        #print(a)
        b=self.linear2.backward(a)
        c=self.sigmoid.backward(b)
        d=self.linear1.backward(c)
        #raise NotImplementedError
    ...
    def compute_loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        for i in range(len(X)):
            y_hat,loss_i=self.forward(X[i], y[i])
            loss+=loss_i
        return loss/len(X)    
        #raise NotImplementedError
    ...
    def train(self, X_tr: np.ndarray, y_tr: np.ndarray,
        ...
        for i in range(n_epochs):
            X,y=shuffle(X_tr,y_tr,i)
            for j in range(len(X)):
                y_hat,loss=self.forward(X[j], y[j])
                self.backward(y[j], y_hat)
                self.step()
    ...
    def test(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        error_rate=1-np.mean(np.array(labels)==y)
