...
def Q(W: np.ndarray, state: np.ndarray, 
    ...
    # TODO: Implement this!
    state_with_bias = np.insert(state, 0, 1.0)
    Q_values = np.dot(W, state_with_bias)
    if action is None:
        return Q_values
    else:
...
if __name__ == "__main__":
    ...
    env_type = "mc"
    mode = "raw"
    weight_out = "mc_tile_weight.txt"
    returns_out = "mc_tile_returns.txt"
    episodes =  2000
    max_iterations = 200
    epsilon = 0.05
    gamma = 0.999
    ...
    # Create environment
    if env_type == "mc":
        env = MountainCar(mode) # TODO: Replace me!
    elif env_type == "gw":
        env = GridWorld(mode) # TODO: Replace me!
    else: 
    ...
    for episode in range(episodes):
        ...
        for iteration in range(max_iterations):
            ...
            #       the bias term!
            Q_value_next=Q(W,next_state)
            Q_value_next_max=np.max(Q_value_next)
            
            gradient_matrix=np.zeros([env.action_space, env.state_space+1], dtype=np.float64)
            gradient_matrix[action,1:]=state
            gradient_matrix[action,0]=1
            W=W-lr*(Q(W,state,action)-reward-gamma*Q_value_next_max)*gradient_matrix
    ...
    mv = [np.mean(rewards[i:i+25]) for i in range(episodes-25)]
    plt.plot(range(episodes), rewards, label="Returns")
    plt.plot(range(25, episodes), mv, label="Rolling Mean")
